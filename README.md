...
## Executar em producao com Docker

O arquivo `docker-compose.prod.yml` inicia a aplicacao e o PostgreSQL com volumes persistentes. Crie um `.env` com senhas fortes usando `.env.example` como referencia e execute:
...
Por padrao, a aplicacao escuta apenas em `127.0.0.1:8080`, pronta para um proxy reverso HTTPS. Em um banco vazio, `INITIAL_ADMIN_*` cria o primeiro administrador. Depois do primeiro login, remova essas tres variaveis do `.env`.

Antes de atualizar, faca backup dos volumes do PostgreSQL e de uploads.